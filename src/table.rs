    ...
    where M: Put<V>
{
    ...
    pub fn put(mut self, hash: SafeHash, key: &str, value: V) -> FullBucket<V, M> {
        unsafe {
            *self.raw.hash() = hash.inspect();
            // self.table.borrow_table_mut().raw_text_data.extend(key.as_bytes());
            ptr::write(self.raw.pair(), (key.to_string(), value));
