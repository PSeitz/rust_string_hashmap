...
#[derive(Clone)]
pub struct HashMap<K, V, S = FnvYoshiBuildHasher> {
...
impl<K: Hash + Eq, V> HashMap<K, V, FnvYoshiBuildHasher> {
    ...
    /// ```
    #[inline]
    pub fn new() -> HashMap<K, V, FnvYoshiBuildHasher> {
        Default::default()
    }
    ...
    /// ```
    #[inline]
    pub fn with_capacity(capacity: usize) -> HashMap<K, V, FnvYoshiBuildHasher> {
          ...
          S: BuildHasher
{
    ...
    /// assert_eq!(map.contains_key(&2), false);
    /// ```
    pub fn contains_key_hashed<Q: ?Sized>(&self, hash: SafeHash, q: &Q) -> bool
        where K: Borrow<Q>,
              Q: Hash + Eq
    {
    ...
    /// assert_eq!(map[&37], "c");
    /// ```
    pub fn insert_hashed(&mut self, hash: SafeHash, k: K, v: V) -> Option<V> {
        self.reserve(1);
        self.insert_hashed_nocheck(hash, k, v)
    }
